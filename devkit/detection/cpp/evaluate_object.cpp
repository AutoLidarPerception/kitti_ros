        ...
        if (fscanf(fp, "%s %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf %lf",
            ...
            for (int c = 0; c < NUM_CLASS; c++) {
                if (!strcasecmp(d.box.type.c_str(), CLASS_NAMES[c].c_str()) ||
                    !strcasecmp(d.box.type.c_str(), CLASS_NAMES_CAP[c].c_str())) {
...
{
    //TODO make sure numbers of frame are equal.
...
bool eval(string result_sha, Mail *mail)
{
    ...
    //TODO hold detections and ground truth in memory
    ...
    for (int32_t i = 0; i < N_TESTIMAGES; i++) {
        ...
        // maintain ground truths & detection results for every frame
